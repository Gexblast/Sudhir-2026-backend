...
// Runs the exact Pine logic on any candle series
function runRsiEmaSignals(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (pineCrossover(rsi, rsiSma, i) && lows[i] <= e200[i] && closes[i] > e200[i]) signals.push({ time: rows[i].time, side: 'BUY', i });
    if (pineCrossunder(rsi, rsiSma, i) && highs[i] >= e200[i] && closes[i] < e200[i]) signals.push({ time: rows[i].time, side: 'SELL', i });
  }
  return { signals, e200 };
}
...
app.get('/rsi-ema-signal', async (req, res) => {
  try {
    ...
    if (rows.length < 210) return res.json({ success: true, index: idx, tf, state: 'NONE', warn: 'Not enough bars yet to seed a 200 EMA on ' + tf + ' — try again shortly', stale: true, strikes: [], reasons: [], checklist: {} });

    const { signals } = runRsiEmaSignals(rows);
    ...
    const state = (last && barsSince <= 3) ? (last.side === 'BUY' ? 'BUY_CE' : 'BUY_PE') : 'NONE';
    const side = state === 'BUY_PE' ? 'PE' : 'CE';
    ...
    // Unified entry-quality score
    let score = 0; const reasons = [];
    if (state !== 'NONE') { score += 40; reasons.push('RSI-SMA cross at 200 EMA fired on ' + tf + ' spot (' + (state === 'BUY_CE' ? 'bullish' : 'bearish') + ')'); }
    const de = data.dealerExposure || {};
    if ((de.dealerHedgePressureScore || 0) > 25) { score += 20; reasons.push('Dealer hedging amplifying (' + de.dealerHedgePressureScore + '/100)'); }
    const of = data.orderFlow || null;
    const ofAligned = of && ((side === 'CE' && of.state === 'STRONG_BULL') || (side === 'PE' && of.state === 'STRONG_BEAR'));
    if (ofAligned) { score += 15; reasons.push('Order flow aligned: ' + of.note); }
    const tr = data.trendRegime || null;
    const trAligned = tr && ((side === 'CE' && tr.regime === 'UPTREND') || (side === 'PE' && tr.regime === 'DOWNTREND'));
    if (trAligned) { score += 15; reasons.push('Multi-day trend aligned (' + tr.regime + ')'); }
    const vx = data.vixRegime || null;
    if (vx && vx.regime && vx.regime.startsWith('SPIKE')) { score += 10; reasons.push('VIX spiking +' + vx.dayChangePct + '% — big-move day'); }

    // Push alert on a FRESH signal only (dedup per index+tf per signal bar)
    if (state !== 'NONE' && last && !stale) {
      const key = idx + '|' + tf;
      if (rsiEmaLastPush.get(key) !== last.time) {
        rsiEmaLastPush.set(key, last.time);
        broadcastPush({
          title: idx + ' ' + (state === 'BUY_CE' ? 'BUY CALL side' : 'BUY PUT side') + ' (' + tf + ')',
          body: 'RSI-SMA cross at 200 EMA. ATM ' + atm + ' · quality ' + score + '/100. Confirm before entering.'
        });
      }
    }
    ...
    res.json({
      success: true, index: idx, tf, state, side, spot: data.spot, atm, barsSince, score, reasons,
      recent: signals.slice(-5).map(s => ({ time: s.time, side: s.side })),
      strikes,
      checklist: {
        signalFired: state !== 'NONE',
        edgePositive: first.edge != null && first.edge >= 0,
        dealer: (de.dealerHedgePressureScore || 0) > 25,
        orderFlow: !!ofAligned,
        trend: !!trAligned,
